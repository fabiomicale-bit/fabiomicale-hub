...
| `motivazione-o-struttura` (CW-2026-03, Batch 3) | LIVE | **SI — https://youtu.be/HCHi7zXCO9Y** | **SI (embed + schema VideoObject, DEPLOYATO su origin/main)** | `DA_VERIFICARE` (materializzata, non inviata) | `DA_VERIFICARE` (Post 2 pronto, non schedulato) | 2026-08-20 | Pubblicato 2026-08-20 su YouTube (upload manuale di Fabio), verificato reale/pubblico via yt-dlp (durata 2:59 = 179s, coerente col render approvato "VIDEO_FINAL_APPROVED.mp4" 2:58,76; titolo YouTube reale "Il problema non è la motivazione. È la struttura", diverso dal titolo bozza preparato — usato il titolo reale). Embed e voce `videoMetadata.ts` committati (`15b5e31`), pushati su `origin/main` e **confermati live** su `https://www.fabiomicale.com/blog/motivazione-o-struttura` (deploy Vercel `dpl_2XGknCwFemw52P5kRKibhXpjETgk`, stato READY, alias di produzione incluso; HTTP 200 e presenza embed/schema riverificati via curl il 2026-08-20). Newsletter e LinkedIn Post 2 già aggiornati con l'URL reale (materiale su Drive, `POST_PUBLICATION_PIPELINE\`), in attesa di gate umano per invio/schedulazione. |
