...
| `motivazione-o-struttura` (CW-2026-03, Batch 3) | LIVE | **SI — https://youtu.be/HCHi7zXCO9Y** | **SI (embed + schema VideoObject)** | `DA_VERIFICARE` (materializzata, non inviata) | `DA_VERIFICARE` (Post 2 pronto, non schedulato) | 2026-08-20 | Pubblicato 2026-08-20 su YouTube (upload manuale di Fabio), verificato reale/pubblico via yt-dlp (durata 2:59 = 179s, coerente col render approvato "VIDEO_FINAL_APPROVED.mp4" 2:58,76; titolo YouTube reale "Il problema non è la motivazione. È la struttura", diverso dal titolo bozza preparato — usato il titolo reale). Embed e voce `videoMetadata.ts` aggiunti in questo ciclo — vedi git log per l'hash del commit e conferma push su `origin/main`. Newsletter e LinkedIn Post 2 già aggiornati con l'URL reale (materiale su Drive, `POST_PUBLICATION_PIPELINE\`), in attesa di gate umano per invio/schedulazione. |
