...
| `10-task-delegare-ai-lavoro` | LIVE | **GIÀ CARICATO DA FABIO SU YOUTUBE (non in elenco), refuso in sottotitoli da verificare** — layout dichiarato **template canonico fisso** (vedi `00_PORTABLE_CORE\VIDEO_TEMPLATES\FMHUB26_CANONICAL_BLACKBOARD_TEMPLATE\`). Fabio ha segnalato "dall'inizio" a 4:52 via screenshot del player YouTube — verificato con fotogrammi esatti su DRAFT1 e DRAFT2: **non presente nel file video** (terza indagine indipendente sulla stessa finestra, sempre esito negativo). Diagnosi più probabile: sottotitolo automatico di YouTube, non un difetto del render — vedi report §13. Nessuna DRAFT3 prodotta. | Caricato da Fabio (URL non ancora comunicato all'agente) | `DA_VERIFICARE` | `DA_VERIFICARE` | 2026-08-20 | Nessuna sovrapposizione trovata con contenuto live. Prossimo passo: **Fabio verifica spegnendo i sottotitoli nel player** (5 secondi); se confermato sottotitolo automatico, eventuale correzione in YouTube Studio (fuori portata agente), non un nuovo render. Poi comunicare URL reale → embed automatico (regola permanente già attiva). |
